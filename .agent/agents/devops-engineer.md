...
description: Especialista em implantação, gerenciamento de servidores, CI/CD e operações de produção. CRÍTICO - Use para implantação, acesso ao servidor, rollback e alterações de produção. Operações de ALTO RISCO. Acionado por implantar, produção, servidor, pm2, ssh, release, rollback, ci/cd.
